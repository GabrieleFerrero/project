        ...
        if azione == "pausa":
            ...
            if lock_esegui.acquire(False):
                ...
                for i, comando_ricevuto in enumerate(comandi_ricevuti.split("\n")):
                    ...
                    comandi_funzioni[comando](parametri, "muovi_motore_con_lock")
...
def muovi_due_motori(comandi, tipo_funzione_muovi_motore):
    ...
    if spostamento[1] == 0:
        for _ in range(abs(spostamento[0])): eval(tipo_funzione_muovi_motore)(index[0] ,asse[0], spostamento[0]//abs(spostamento[0]))
    elif spostamento[0] == spostamento[1]:
        for _ in range (abs(spostamento[0])):
            eval(tipo_funzione_muovi_motore)(index[0], asse[0], abs(spostamento[0])//spostamento[0]) 
            eval(tipo_funzione_muovi_motore)(index[1], asse[1], abs(spostamento[1])//spostamento[1])
    else: 
        #------CALCOLI-------#
        ...
        for a in range (abs(spostamento[1])):
            for i in range (impulsi_arrotondati):
                if i == impulsi_arrotondati//2:
                    eval(tipo_funzione_muovi_motore)(index[1], asse[1], spostamento[1]//abs(spostamento[1]))
                
                if ogni_quanto_impulso!= 0 and (i+a*impulsi_arrotondati)%ogni_quanto_impulso == 0 and differenza_impulsi!=0:
                    eval(tipo_funzione_muovi_motore)(index[0] ,asse[0], differenza_impulsi//abs(differenza_impulsi))
                    differenza_impulsi -= differenza_impulsi//abs(differenza_impulsi)
                
                eval(tipo_funzione_muovi_motore)(index[0] ,asse[0], spostamento[0]//abs(spostamento[0]))
...
def arco(parametri, tipo_funzione_muovi_motore):
    ...
    for ang in np.linspace(angolo_iniziale,angolo_finale,int((angolo_finale-angolo_iniziale)*100)):
        vai_lineare([coordx_centro+raggio*np.cos(ang), coordy_centro+raggio*np.sin(ang), 0], tipo_funzione_muovi_motore)
    """---------------------------"""
    """dal centro dell'arco, mi alzo un po' la z e mi sposto in un punto della circonferenza"""
