public class Principale extends PrincipaleProcessing{
    ...
    public void creaJoyStick(float centro_x, float width_tabella, boolean isJoyStickSinistra){
        strokeWeight(2);
        stroke(24,159,151);
        fill(24,159,151);
        //float centro_x = (width-width_tabella)/4;
        pushMatrix();
        translate(centro_x-(width-width_tabella)/8, (float) ((height*3/5)-50),-2);
        box((width-width_tabella)/4, 100, -2);
        popMatrix();

        if(isJoyStickSinistra){
            pushMatrix();
            translate(centro_x-50, (float) (height*3/5)-(float)height/8,-2);
            box(100, (float)(height/4), -2);
            popMatrix();
        }
        fill(244,159,151);
